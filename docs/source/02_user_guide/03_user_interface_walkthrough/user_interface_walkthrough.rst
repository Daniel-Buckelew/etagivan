...
The autofocus menu has two options: :guilabel:`Perform Autofocus`, which autofocus the sample using the current autofocus settings, and :guilabel:`Autofocus Settings`, which launches the :ref:`Autofocus Settings <ui_autofocus>` popup.
...
* :guilabel:`Time Interval (hh:mm:ss)` provides an estimate of how long each time point takes to acquire. This is (stack acquisition + stack pause) x number of channels to
  image.
...
The :guilabel:`Stage Control` Settings Notebook is a tab (optionally, a popup) that controls the stage positions. It is split into six parts: :guilabel:`Stage Positions`,
:guilabel:`X Y Movement`, :guilabel:`Z Movement`, :guilabel:`Focus Movement`, :guilabel:`Theta Movement`, and includes an emergency :guilabel:`STOP` button, as well
...
not use that control. See the :ref:`stage subsection <configure_stages>`
...
the joystick (see the :ref:`stage subsection <configure_stages>`).
...
* :guilabel:`Launch Tiling Wizard` launches the
  :ref:`Tiling Wizard <ui_multiposition_tiling_wizard>`
...
:ref:`Channel Settings <ui_channel_settings>`
...
:guilabel:`Waveform Template` changes the :ref:`waveform template <configure_waveform_templates>`
...
:ref:`acquisition mode <ui_acquisition_bar>` other than
"Continuous Scan" is selected
and :ref:`Save Data <ui_timepoint_settings>` is checked.
...
:ref:`Waveform Settings <ui_waveform_settings>`.
...
* :guilabel:`% Overlap` indicates the percent of the the that should overlap along each axis. It is a percent of the FOV Dist.
...
:ref:`Tiling a sample larger than the field of view <acquire_mesospimbt_tiling>`.
...
:ref:`feature <user_guide_features>`.
